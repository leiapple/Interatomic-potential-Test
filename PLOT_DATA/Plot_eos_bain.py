...
eos_gap21 = pd.read_csv("./eos_gap_lei.csv",delimiter=" ",decimal=".",header=None,names=["volume","energy"])
...
bp_gap21 = pd.read_csv("./BainPath_GAP_lei.csv",delimiter=" ",decimal=".",header=None,names=["ratio","energy","ratio2","lx","ly","lz"])
...
# plot EV curve
ax.set_title('Energy volume curve',fontsize=16)
ax.scatter(eos_dft["volume"], eos_dft["energy"], s=30, marker='o', c='dodgerblue', alpha=1,label='DFT')
ax.scatter(eos_gap18["volume"], eos_gap18["energy"],  s=30, marker='p', c = 'lime', alpha=1,label='GAP18')
ax.scatter(eos_gap21["volume"], eos_gap21["energy"],  s=30, marker='s', c = 'red', alpha=1,label='GAP21')
ax.plot(eos_dft["volume"], eos_dft["energy"],lw=1.5,alpha=1)
ax.plot(eos_gap18["volume"], eos_gap18["energy"],lw=1.5,alpha=1)
ax.plot(eos_gap21["volume"], eos_gap21["energy"],lw=1.5,alpha=1)

ax.set_xlabel('Volume, [$\AA^3$]',fontsize='16')
ax.set_ylabel('Energy, [meV]',fontsize='16')
ax.legend(loc='upper center',fontsize=16,markerscale=1.5)


# Plot right side
bx.set_title('Bain path',fontsize=16)
bx.scatter(bp_dft["ratio"],bp_dft["energy"],s=30, marker='o', c='dodgerblue', alpha=1,label='DFT')
bx.scatter(bp_gap18["ratio"], bp_gap18["energy"],  s=30, marker='p', c = 'lime', alpha=1,label='GAP18')
bx.scatter(bp_gap21["ratio"], bp_gap21["energy"],  s=30, marker='s', c = 'red', alpha=1,label='GAP21')
bx.plot(bp_dft["ratio"], bp_dft["energy"],lw=1.5,alpha=1)
bx.plot(bp_gap18["ratio"], bp_gap18["energy"],lw=1.5,alpha=1)
...
bx.set_xlabel('c/a ratio',fontsize='16')
bx.legend(loc='upper center',fontsize=16,markerscale=1.5)

for label in (ax.get_xticklabels() + ax.get_yticklabels() + bx.get_xticklabels() + bx.get_yticklabels()):
	label.set_fontsize(16)

fig.savefig('gap21_eos_bp.png',dpi=300)